...
| 3 | Persistent block entities | Phases 1-2 | Partial | `Code`, `Compile`, `Runtime` | `desk`, `waypoint`, `crane`, the small `shipyard`, and now `volcore` / `polymetal` / `heavy grudge` all run on modern block entities. Large shipyard structure logic remains pending. |
...
| Pointer item | Command-scepter mode handling | Partial | `Code`, `Compile` | `item/PointerItem.java`, `teitoku/TeitokuHelper.java` | Single/group escort toggle, caress mode, and target-class editing are restored. Full team/formation packet flows plus guard/attack command families are still pending. |
...
| Legacy core heavy blocks | Volcore/polymetal/heavy grudge owner + mode + persistent charge baseline | Partial | `Code`, `Compile` | `block/LegacyCoreBlock.java`, `blockentity/LegacyCoreBlockEntity.java`, `registry/ModBlockEntities.java` | The three heavy blocks are no longer message-only placeholders: they now persist owner, operating mode, and charge state with server tick updates. Multiblock/automation loops are still pending. |
